const consultas = {
    ...
    getFoodInventoryHistory: "SELECT id_food, name , mark, weight, price, date, fw.amount as cantidad, id_food_inventory FROM food_warehouse fw JOIN food_inventory fi ON fw.id_food_inventory = fi.id JOIN foods f ON fi.id_food = f.id WHERE id_food = ? AND id_farm = ?",
    getLots: "Select * from lots where id_shed = ?",
